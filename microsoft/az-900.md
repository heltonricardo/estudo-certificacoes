...
2. Selecione uma delas
3. Clique em `Conectar` > `Bastion`
4. Insira suas credenciais de usuário escolhidas durante a criação da VM
5. Clique em `Conectar` (uma nova guia do navegador será aberta com o terminal da máquina)
6. Execute o comando `sudo su`
7. Execute o comando `ifconfig` (caso precise instalar, execute: `apt install net-tools`)
8. Procure pelo endereço que segue a palavra `inet` na seção `eth0` - provalvelemente será `10.1.0.4` - esse é o endereço IP da VM (a Azure costuma utilizar os 3 primeiros IPs disponíveis dentro de cada rede). O IP da máquina virtual também é informado no painel da Azure: depois de selecionar a VM, procure a seção `Propriedades`
9. Execute o comando `ping 8.8.8.8` e verifique se ela possui acesso à internet
10. Execute o comando `ping <X>` (onde <X> é o IP da segunda VM) e verifique se ela possui acesso à outra máquina da mesma VNET
